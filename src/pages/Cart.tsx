  ...
  if (items.length === 0 && !showCheckout) {
    return (
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-32 text-center">
        ...
        <h1 className="text-5xl font-black tracking-tighter mb-6">Seu carrinho está vazio :(</h1>
        ...
        <Link 
          to="/" 
          className="inline-flex items-center bg-brand-black text-white px-10 py-5 rounded-full font-black uppercase tracking-widest text-sm hover:scale-105 transition-transform"
        >
          Voltar para a Loja
        </Link>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-10 py-16">
      <div className="flex justify-between items-center mb-16">
        ...
        {showCheckout ? (
          <button 
            ...
            className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-gray-400 hover:text-brand-black transition-colors"
          >
          ...
          <button 
            onClick={clearCart}
            className="text-xs font-black uppercase tracking-widest text-gray-400 hover:text-brand-red flex items-center transition-colors"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
              ...
              <AnimatePresence mode="popLayout">
                {items.map((item, idx) => (
                  <motion.div
                    ...
                    className="flex items-center gap-6 bg-brand-gray p-6 rounded-[40px] shadow-sm group border-4 border-transparent hover:border-brand-yellow transition-all"
                  >
                    ...
                    <div className="flex-grow">
                      ...
                      {item.customization && (
                        <div className="mt-1 flex flex-col gap-1">
                          <p className="text-[10px] font-black text-brand-red uppercase">✨ Produto Personalizado</p>
                    ...
                    <div className="text-right flex flex-col items-end gap-3">
                      <span className="font-black text-xl text-brand-red">{formatPrice(item.price * item.quantity)}</span>
                      <button 
                        onClick={() => removeItem(item.id, item.customization)}
                        className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-gray-400 hover:bg-brand-red hover:text-white transition-all shadow-sm"
                      >
                        <X className="w-5 h-5" />
                      </button>
                    </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-brand-black text-white p-10 rounded-[50px] sticky top-32 shadow-2xl overflow-hidden border-2 border-brand-yellow/30">
            ...
            <div className="space-y-6 mb-12">
              ...
              <div className="pt-6 border-t border-white/10 flex justify-between items-end">
                ...
                <span className="text-4xl font-black text-brand-yellow tracking-tighter">{formatPrice(finalTotal)}</span>
              </div>
            </div>
            {!showCheckout && (
              <button 
                className="w-full bg-brand-red text-white py-6 rounded-full font-black uppercase tracking-[0.2em] text-xs hover:scale-105 transition-transform flex items-center justify-center group"
                onClick={() => {
